...
function AnalyticsSelect({
  ...
  return (
    <div className={`an-reactSelectWrap ${triggerClassName}`.trim()}>
      <ReactSelect
...
export default function AnalyticsPage({ userId = "default_user", subjectId = "default_subject" }) {
  ...
  const masteryGap =
    topWeak && topStrong
      ? Math.max(0, Number(topStrong.accuracy || 0) - Number(topWeak.accuracy || 0))
      : null;
  ...
  const openReviewPopup = async (row, source = "queue") => {
    ...
    try {
      ...
      const scoreRaw = source === "history"
        ? row.score_percent
        : (row.latest_score_percent ?? row.score_percent);
    ...
    <div className="analyticsShell">
      ...
      <section className="an-card an-summaryCard">
        ...
        <div className="an-summaryGrid">
          ...
          <div className="an-kpi">
            ...
            <div className="an-cardHint">Longest consecutive correct attempts (all time for this subject).</div>
          </div>
        </div>
      </section>
      ...
      <section className="an-card">
        <div className="an-cardHead">
          ...
          <div className="an-cardHint">Single smoothed line. Window: {overview.rolling_window || rollingWindow} attempts.</div>
        </div>
          ...
          <div className="an-controlBlock">
            ...
            <AnalyticsSelect
              ...
              options={coverageFiles.map((f) => ({
                value: f.file_id,
                label: truncateText(f.file_name, 54),
              }))}
            />
          </div>
        </div>
        ...
        <div className="an-card">
          ...
          <div className="an-tableWrap">
            <table className="an-table">
              ...
              <tbody>
                {weakItems.length === 0 ? (
                  <tr><td colSpan={4} className="an-mutedCell">No weak areas yet.</td></tr>
                ) : weakItems.map((x) => (
                  <tr key={`${x.type}:${x.key}`}>
            ...
            <table className="an-table">
              ...
              <tbody>
                {strongItems.length === 0 ? (
                  <tr><td colSpan={4} className="an-mutedCell">No strong areas yet.</td></tr>
                ) : strongItems.map((x) => (
                  <tr key={`${x.type}:${x.key}`}>
                    <td className="an-muted">{truncateText(areaValueLabel(x.type, x.key))}</td>
                    <td className="an-muted">{areaTypeLabel(x.type)}</td>
        ...
        <div className="an-miniList">
          ...
          <div className="an-miniRow">
            ...
            <div className="an-miniRight">
              {topWeak
                ? `${truncateText(areaValueLabel(topWeak.type, topWeak.key), 32)} (${percent(topWeak.accuracy)})`
                : "-"}
            </div>
          </div>
          <div className="an-miniRow">
            ...
            <div className="an-miniRight">
              {topStrong
                ? `${truncateText(areaValueLabel(topStrong.type, topStrong.key), 32)} (${percent(topStrong.accuracy)})`
                : "-"}
            </div>
          </div>
          <div className="an-miniRow">
          ...
          <table className="an-table">
            ...
            <tbody>
              {reviewQueue.length === 0 ? (
                <tr><td colSpan={3} className="an-mutedCell">No review queue yet.</td></tr>
              ) : reviewQueue.slice(0, 10).map((x) => (
                  <tr
                    key={x.question_id}
            ...
            <table className="an-table">
              ...
              <tbody>
                {historyItems.length === 0 && !historyLoading ? (
                  <tr><td colSpan={6} className="an-mutedCell">No attempts yet.</td></tr>
                ) : historyItems.map((h) => (
                  <tr
                    key={h.attempt_id}
                    className="an-clickableRow"
